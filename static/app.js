...
elModalAdmin.addEventListener('click', (e) => {
  if (e.target === elModalAdmin) closeAdminModal();
});
...
const elAgregarBtnText = document.getElementById('agregar-btn-text');
const elAgregarSpinner = document.getElementById('agregar-spinner');
...
async function submitAgregar() {
  ...
  elAgregarBtnText.classList.add('hidden');
  elAgregarSpinner.classList.remove('hidden');
  elAgregarSubmit.disabled = true;

  try {

    const res = await fetch('/admin/agregar', {
      method: 'POST',
      body: formData
    });
    ...
    elAgregarBtnText.classList.remove('hidden');
    elAgregarSpinner.classList.add('hidden');
    elAgregarSubmit.disabled = false;
  }
}

elAgregarSubmit.addEventListener('click', submitAgregar);
